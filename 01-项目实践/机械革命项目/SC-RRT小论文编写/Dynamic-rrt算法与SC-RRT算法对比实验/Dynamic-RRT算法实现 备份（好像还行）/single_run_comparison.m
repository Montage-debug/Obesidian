...
for alg_idx = 1:num_algorithms
    ...
    title(alg.name, 'FontSize', 12, 'FontWeight', 'bold');
    ...
    if result.success
        ...
        % 添加性能文本
        text_str = sprintf('时间: %.3fs\n长度: %.1fmm\n节点: %d\n平滑度: %.4f', ...
                          result.time, result.length, result.nodes, result.smoothness);
        text(100, 1400, text_str, ...
             'FontSize', 9, ...
             'BackgroundColor', 'w', ...
             'EdgeColor', 'k', ...
...
for i = 1:num_algorithms
    if results{i}.success
        success_idx(end+1) = i;
        success_names{end+1} = algorithms{i}.name;
        success_times(end+1) = results{i}.time;
        success_lengths(end+1) = results{i}.length;
        success_nodes(end+1) = results{i}.nodes;
    end
end

if ~isempty(success_idx)
    % 归一化到[0,1]
    norm_times = success_times / max(success_times);
    norm_lengths = success_lengths / max(success_lengths);
    norm_nodes = success_nodes / max(success_nodes);
    
    % 绘制分组柱状图
    x = 1:length(success_idx);
    width = 0.25;
    
    bar(x - width, norm_times, width, 'FaceColor', [0.2 0.6 0.8]);
    bar(x, norm_lengths, width, 'FaceColor', [0.8 0.4 0.2]);
    bar(x + width, norm_nodes, width, 'FaceColor', [0.4 0.7 0.4]);
    
    set(gca, 'XTick', x, 'XTickLabel', success_names, 'XTickLabelRotation', 15);
    ylabel('归一化值', 'FontSize', 10, 'FontWeight', 'bold');
    title('性能对比 (归一化)', 'FontSize', 12, 'FontWeight', 'bold');
    legend('时间', '路径长度', '节点数', 'Location', 'best', 'FontSize', 9);
    ylim([0 1.2]);
    set(gca, 'FontSize', 9);
else
    text(0.5, 0.5, '无成功算法', ...
