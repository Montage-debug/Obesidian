...
%
% 核心特性:
%   1. Informed subset采样: 使用F̂(c)估计构建超椭球采样空间
...
% 初始化树结构
tree = struct();
tree.nodes = [startPoint];        % 节点位置 [n×m]
tree.parents = [0];                % 父节点索引
tree.costs = [0];                  % 从start到该节点的累计cost
tree.children = {[]};              % 子节点列表（用于计算out-degree）
tree.count = 1;
...
for iter = 1:maxIterations
    % 1. 找到离goal最近的节点c（用于计算F̂(c)）
    distances = vecnorm(tree.nodes - repmat(goalPoint, tree.count, 1), 2, 2);
    ...
    % 6. 周期性Pareto选择新起点
    if counter >= interval
        % 执行Pareto dominance选择
        [newStartIdx, newStart] = ChooseNodePareto(tree, goalPoint, paretoProb, currentStartIdx);
        
        % 检查从当前start到新start的路径是否无碰撞
        if ~CheckCollision(currentStart, newStart, obstacles, m)
            currentStart = newStart;
            currentStartIdx = newStartIdx;
            if enableVisualization && m == 2
                plot(currentStart(1), currentStart(2), 'mo', 'MarkerSize', 8, 'MarkerFaceColor', 'm');
            elseif enableVisualization && m == 3
                plot3(currentStart(1), currentStart(2), currentStart(3), 'mo', 'MarkerSize', 8, 'MarkerFaceColor', 'm');
            end
        end
