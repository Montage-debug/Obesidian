...
% RRT*参数 — 与 RRT_Star_Basic 保持一致的 gamma 计算
gamma_rrt_star = norm(goal_point - start_point) * 2;
...
goal_node_in_tree = false;  % 目标点是否已加入树
goal_tree_idx = -1;         % 目标点在树中的索引
first_solution_time = inf;  % 首次找到解的实际时间
...
for iter = 1:max_iterations
    ...
    % 重布线后同步 c_best（重布线可能通过子节点传播改善了到目标的代价）
    if goal_node_in_tree
        c_best = tree_cost(goal_tree_idx);
    end
    ...
    if dist_to_goal < goal_threshold
        if isCollisionFreeLocal(new_point, goal_point, obstacles, dim)
            ...
            if ~goal_node_in_tree
                ...
                tree_nodes(n_nodes, :) = goal_point;
                tree_parent(n_nodes) = n_nodes - 1;  % 父节点是刚添加的新节点
                tree_cost(n_nodes) = path_cost_to_goal;
                goal_tree_idx = n_nodes;
                goal_node_in_tree = true;
                goal_idx = n_nodes;
                ...
                % 已有目标节点：若经新节点到目标更优则更新父节点
                if path_cost_to_goal < tree_cost(goal_tree_idx)
                    tree_parent(goal_tree_idx) = n_nodes;
                    tree_cost(goal_tree_idx) = path_cost_to_goal;
                    c_best = path_cost_to_goal;
                end
            end
        end
...
%% ========== 路径提取 ==========
if success && goal_tree_idx > 0
    % 从目标节点回溯最优路径（目标已在树中）
    path = [];
    current = goal_tree_idx;
    while current > 0
...
if success
    ...
    tree.convergence_time = first_solution_time;
else
    tree.final_cost = inf;
