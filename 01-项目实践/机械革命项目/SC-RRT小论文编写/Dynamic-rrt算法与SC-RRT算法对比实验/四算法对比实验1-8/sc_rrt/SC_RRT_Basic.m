...
if success && ~isempty(path)
    ...
    try
        path = shortcutPath(path, obstacles, dim, 25);
    catch
    end
    
    % Step 2: 弹性带拉直优化 - 将路径节点拉向局部最优位置（增强迭代）
    try
        path = pullPathToOptimal(path, obstacles, dim, 40);
    catch
    end
    ...
    try
        path = smoothPathSimple(path, obstacles, dim, 20);
    catch
    end
    ...
    try
        ...
        [path_fillet, fillet_ok] = smoothPathWithFillets(path, obstacles, dim, fillet_r, 30);
