...
%% ========== 在线参数自适应调节器（Contextual Bandit + UCB） ==========
tuner = OnlineParamTuner('UCB_C', 1.5, 'UpdateInterval', 30, 'DecayFactor', 0.95);
fprintf('  [OnlineParamTuner] 已初始化: %d个臂, UCB探索系数=%.1f\n', tuner.num_arms, tuner.ucb_c);
...
while iterations < max_iterations && ~success
    ...
    if mod(iterations, update_interval) == 0 && sizeA > 1 && sizeB > 1
        ...
        % 在线学习：更新参数调节器并获取自适应gamma
        if tuner.shouldUpdate(iterations)
            tuner.updateParams(sizeA, sizeB);
            [~, ~, ~, gamma_tuned] = tuner.getSamplingParams();
            ellipsoid_buffer = gamma_tuned;  % 用学习到的gamma替代固定值
        end
        
        % 更新双椭球体约束（使用自适应ellipsoid_buffer）
    ...
    % ===== 2. 扩展树A（从起点向交汇点） =====
    % 在线自适应采样策略（由Contextual Bandit调节）
    [p_meet_now, p_cross_now, ~, ~] = tuner.getSamplingParams();
    r_val = rand;
    if r_val < p_meet_now
        sample_A = meet_point;  % 自适应概率采样交汇点
    elseif r_val < p_meet_now + p_cross_now
        % 自适应概率采样树B最近节点附近（促进快速连接）
        if sizeB > 1
    ...
    % 碰撞检测（带在线学习记录）
    collision_free_A = isCollisionFree(nearest_point_A, new_point_A, obstacles, dim);
    tuner.recordSample(~collision_free_A);
    if collision_free_A
        ...
        tuner.recordCostUpdate(best_cost_A);  % 记录路径代价用于在线学习
    ...
    collision_free_B = isCollisionFree(nearest_point_B, new_point_B, obstacles, dim);
    tuner.recordSample(~collision_free_B);
    if collision_free_B
        % 找邻域内的节点（RRT*思想）
        radius = min(step_size * 2.5, norm(goal_point - start_point) / 20);
        ...
        tuner.recordCostUpdate(best_cost_B);  % 记录路径代价用于在线学习
...
% 输出在线学习调节结果
tuner_info = tuner.getDebugInfo();
fprintf('SC-RRT完成: %s, iter=%d, A=%d, B=%d, t=%.3fs\n', ...
    string(success), iterations, sizeA, sizeB, planning_time);
fprintf('  [OnlineParamTuner] 最终参数: p_meet=%.2f, p_cross=%.2f, gamma=%.2f (拉动%d次)\n', ...
    tuner_info.current_params(1), tuner_info.current_params(2), tuner_info.current_params(3), tuner_info.total_pulls);
...
% 记录在线学习最终参数
tuner_final = tuner.getDebugInfo();
metrics.tuner_final_params = tuner_final.current_params;  % [p_meet, p_cross, gamma]
metrics.tuner_total_pulls = tuner_final.total_pulls;
