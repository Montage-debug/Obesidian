classdef PerformanceCollector < handle
    ...
    methods
        ...
        function addResult(obj, algorithm_name, env, path, tree, success, metrics)
            ...
            fprintf('✓ 已添加结果: %s [%dD, 障碍物:%d, 成功:%d, 时间:%.3fs]\n', ...
        ...
        function summary = getSummary(obj)
            ...
            for i = 1:length(obj.algorithms)
                ...
                summary.(obj.safeFieldName(algo)) = struct(...
                    ...
                    'avg_iterations', mean(cellfun(@(x) x.iterations, algo_results), 'omitnan'), ...
                    'avg_tree_nodes', mean(cellfun(@(x) x.tree_nodes, algo_results), 'omitnan'), ...
                    'avg_path_length', mean(cellfun(@(x) x.path_length, success_results), 'omitnan'), ...
                    'avg_planning_time', mean(cellfun(@(x) x.planning_time, algo_results), 'omitnan'), ...
                    'avg_smoothness', mean(cellfun(@(x) x.smoothness, algo_results), 'omitnan'), ...
                    'avg_clearance', mean(cellfun(@(x) x.clearance, algo_results), 'omitnan'), ...
                    'avg_path_efficiency', mean(cellfun(@(x) x.path_efficiency, success_results), 'omitnan'), ...
                    'avg_node_efficiency', mean(cellfun(@(x) x.node_efficiency, success_results), 'omitnan'), ...
                    'avg_turning_angle', mean(cellfun(@(x) x.avg_turning_angle, success_results), 'omitnan'), ...
                    'max_turning_angle', max(cellfun(@(x) x.max_turning_angle, success_results)), ...
                    'avg_path_density', mean(cellfun(@(x) x.path_density, success_results), 'omitnan'), ...
                    'min_clearance', min(cellfun(@(x) x.min_clearance, success_results)), ...
                    'avg_avg_clearance', mean(cellfun(@(x) x.avg_clearance, success_results), 'omitnan') ...
                );
            end
        end
        ...
        function printSummary(obj)
            ...
            for i = 1:length(algo_names)
                ...
                fprintf('  平均平滑度:   %.4f\n', s.avg_smoothness);
                fprintf('  平均间隙:     %.2f\n', s.avg_clearance);
                fprintf('  路径效率:     %.4f\n', s.avg_path_efficiency);
                fprintf('  节点利用率:   %.4f\n', s.avg_node_efficiency);
                fprintf('  平均转角:     %.2f°\n', s.avg_turning_angle);
                fprintf('  最小安全间隙: %.2f\n', s.min_clearance);
                fprintf('\n');
            end
