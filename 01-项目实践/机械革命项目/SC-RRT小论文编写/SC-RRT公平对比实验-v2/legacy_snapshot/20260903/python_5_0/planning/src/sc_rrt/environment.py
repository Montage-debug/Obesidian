...
class EnvironmentConfig:
    ...
    def generate_2d_environment(
        ...
        if max_attempts is None:
            max_attempts = max(EnvironmentConfig.MAX_PLACEMENT_ATTEMPTS, num_obstacles * 40)

        obstacles = []
        radius_min, radius_max = radius_range
        rounds = max_adjust_rounds if auto_adjust else 1
        ...
        for round_idx in range(rounds):
            ...
            while len(obstacles) < num_obstacles and attempts < max_attempts:
                ...
                radius = radius_min + np.random.rand() * (radius_max - radius_min)
            ...
            radius_min = max(min_radius_limit, radius_min * shrink_factor)
            radius_max = max(radius_min + 0.5, radius_max * shrink_factor)

        if len(obstacles) < num_obstacles:
            print(
                f"警告: 只成功放置了 {len(obstacles)}/{num_obstacles} 个障碍物，"
                f"已尝试缩小半径范围至({radius_min:.1f}, {radius_max:.1f})"
            )
        ...
        return {
            ...
            'num': len(obstacles),
            'radius_range_used': (radius_min, radius_max)
        }
    ...
    def generate_3d_environment(
        ...
        if max_attempts is None:
            max_attempts = max(EnvironmentConfig.MAX_PLACEMENT_ATTEMPTS, num_obstacles * 35)
