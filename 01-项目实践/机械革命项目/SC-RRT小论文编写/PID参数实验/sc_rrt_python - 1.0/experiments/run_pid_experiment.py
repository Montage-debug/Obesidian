...
class PIDExperiment:
    ...
    def __init__(
        ...
        # 实验配置 - 增加迭代冗余以提升成功率至60%+
        if quick_test:
            self.num_trials = 30  # 大批量实验：30次重复提升统计可靠性
            self.max_iterations_2d = 750   # 二维：750次 [避免压线成功，给PID充足空间]
            self.max_iterations_3d = 950   # 三维：950次 [避免压线成功，体现真实效率差距]
            print("【大批量实验模式 - 最终优化配置】\n")
            print(f"  二维场景: {self.max_iterations_2d} 次迭代 [目标：90-95%成功率，体现PID效率]")
            print(f"  三维场景: {self.max_iterations_3d} 次迭代 [目标：避免压线，真实Sample-to-Success]")
    ...
    def _create_pid_configs(self) -> List[Dict]:
        ...
        # 标准快速测试模式
        if self.quick_test:
            # 快速测试: 9个参数组，从极低到极高，跨度大
            return [
                # 极低参数组（欠阻尼，探索不足）
                {'Kp': 0.05, 'Ki': 0.01, 'Kd': 0.02, 'name': 'P1_VeryLow'},
                {'Kp': 0.10, 'Ki': 0.02, 'Kd': 0.04, 'name': 'P2_Low'},
                {'Kp': 0.20, 'Ki': 0.03, 'Kd': 0.08, 'name': 'P3_Low-Mid'},
                
                # 中等参数组（平衡探索与利用）
                {'Kp': 0.30, 'Ki': 0.05, 'Kd': 0.12, 'name': 'P4_Mid'},
                {'Kp': 0.40, 'Ki': 0.07, 'Kd': 0.16, 'name': 'P5_Mid-High'},
                {'Kp': 0.50, 'Ki': 0.10, 'Kd': 0.20, 'name': 'P6_High'},
                
                # 高参数组（强探索，可能过冲）
                {'Kp': 0.60, 'Ki': 0.12, 'Kd': 0.24, 'name': 'P7_VeryHigh'},
                {'Kp': 0.70, 'Ki': 0.15, 'Kd': 0.28, 'name': 'P8_ExtremeHigh'},
                {'Kp': 0.80, 'Ki': 0.18, 'Kd': 0.32, 'name': 'P9_Maximum'}
            ]
        else:
            # 完整实验: 33个参数组（精细网格搜索）
            configs = []
