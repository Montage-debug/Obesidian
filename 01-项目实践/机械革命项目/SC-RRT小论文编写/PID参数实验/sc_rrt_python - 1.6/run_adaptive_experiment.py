# -*- coding: utf-8 -*-
"""
SC-RRT Adaptive Sampling Control Experiment
SC-RRT 自适应采样控制实验

Usage:
    Quick Test (15-20 min):  python run_adaptive_experiment.py --quick
    Full Experiment (1-2h):  python run_adaptive_experiment.py
"""
...
from experiments.run_adaptive_comparison import AdaptiveComparisonExperiment
...
def run_baseline_experiment():
    ...
    print("  2️⃣  达到固定成功率所需采样�?   - 证明PID提高单位采样价�?)
    print("  3️⃣  椭球体收缩稳定�?          - 证明PID控制约束机制可控可用")
    print("\n【预期结�?- 高密度障碍物环境�?)
    print("  成功率对�?")
    print("    �?No_PID: 25-35% (无引导，随机搜索效率�?")
    print("    �?Optimal_PID: 50-65% (PID优化引导，成功率提升2�?")
    print("  效率指标:")
    print("    �?ESR: No_PID=0% vs Optimal_PID~45-60%")
    print("    �?Sample-to-Success: No_PID~800�?vs Optimal_PID~400�?)
    print("\n【实验配置�?)
    print("  配置�? 4 �?)
    print("  场景�? 2 个（二维225@R22-38 + 三维400@R35-55，自适应缩放�?)
    print("  重复次数: 6 �?配置（稳定性预评估�?)
    print("  迭代限制: 2D=900, 3D=1200")
    print("  总运行数: 4 × 2 × 6 = 48 �?)
    print("  预计耗时: 6-10分钟")
    print("="*70)
...
def run_refined_search():
    ...
    print("  - 2 scenarios (2D-225obs, 3D-400obs)")
    print("  - 6 repeats per group")
    print("  - Total runs: 14 x 2 x 6 = 168")
...
def run_fine_tuning():
    ...
    print("  - 2 scenarios (2D-225obs, 3D-400obs)")
    print("  - 6 repeats per group")
    print("  - Total runs: 12 x 2 x 6 = 144")
    print("  - Estimated time: 10-15 minutes")
    
    # 创建实验对象
    experiment = AdaptiveComparisonExperiment(
        quick_test=True,  # 使用快速模式的重复次数
        fine_tuning=True,  # 启用三参数精细调�?
        output_dir='results'
    )
...
def run_quick_test():
    ...
    print("  - 2 scenarios (2D-225obs, 3D-400obs)")
    print("  - 6 repeats per group [STABILITY CHECK]")
    print("  - 2D: 900 iterations, 3D: 1200 iterations")
    print("  - Total runs: 10 x 2 x 6 = 120")
    print("  - Estimated time: 12-18 minutes")
    
    # 创建实验对象
    experiment = AdaptiveComparisonExperiment(
        quick_test=True,  # 使用优化的迭代配�?
        output_dir='results'
    )
...
def run_full_experiment():
    ...
    print("  - 2 scenarios (2D-225obs, 3D-400obs)")
    print("  - 12 repeats per group [MID-SCALE]")
    print("  - 2D: 1200 iterations, 3D: 1500 iterations")
    print("  - Total runs: 9 x 2 x 12 = 216")
...
def analyze_and_visualize(results_df, experiment):
    ...
    # 稳定性评估（决定是否可扩大实验规模）
    print("\n[STABILITY] Assessing stability (path quality based)...")
    try:
        # 基于成功率和样本量评�?
        if optimal_pid is not None:
            success_rate = optimal_pid.get('success_rate_pct', 0) / 100.0
            if success_rate >= 0.30:
                print("[STABLE] �?成功率达�?0%以上，结果可�?)
                print("         建议：可以进行参数对比分�?)
            else:
                print("[UNSTABLE] �?成功率低�?0%")
                print("         建议：降低任务难度或调整参数范围")
        else:
            print("[WARNING] 无法评估稳定�?)
    except Exception as e:
        print(f"[WARNING] Stability assessment skipped: {e}")
...
def main():
    """主函�?""
    # 解析命令行参�?
    parser = argparse.ArgumentParser(description='SC-RRT PID有效性验证实�?- 采样行为分析')
    parser.add_argument('--quick', action='store_true', help='快速实�?(240 runs, ~15-18分钟)')
    parser.add_argument('--refine', action='store_true', help='精细化参数搜�?(280 runs, ~20-25分钟)')
    parser.add_argument('--finetune', action='store_true', help='三参数调�?(240 runs, ~15-18分钟)')
    parser.add_argument('--baseline', action='store_true', help='【推荐】PID有效性验证实�?(240 runs, ~15-18分钟)')
