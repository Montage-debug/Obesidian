...
class ResultAnalyzer:
    ...
    def generate_summary_statistics(self):
        ...
        if len(df_success) > 0:
            ...
            # 计算变异系数(CV) - 路径质量评价模式的核心指标
            success_stats['path_length_cv'] = (success_stats['path_length_std'] / success_stats['path_length_mean'] * 100).round(2)
            success_stats['planning_time_cv'] = (success_stats['planning_time_std'] / success_stats['planning_time_mean'] * 100).round(2)
        ...
        # 如果有成功样本，显示路径质量
        if len(df_success) > 0:
            print("\n🎯 路径质量评价模式 - 成功样本的质量与稳定性指标:")
            quality_cols = ['pid_config', 'path_length_mean', 'path_length_std', 'path_length_cv',
                          'path_efficiency_mean', 'planning_time_mean', 'planning_time_cv']
            print(grouped[quality_cols].dropna().to_string(index=False))
            print("\n说明: CV(变异系数)越小表示稳定性越好")
    ...
    def find_optimal_pid(self, success_threshold=0.55):
        """
        中等难度评价模式 - 在成功率有区分度时比较综合性能
        
        Args:
            success_threshold: 成功率阈值（默认55%，适应中等难度，能拉开差距）
        
        Returns:
            最优PID配置及分析报告
        """
        print("\n" + "="*70)
        print(f"🎯 中等难度评价模式（成功率阈值≥{success_threshold*100:.0f}%）")
        print("="*70)
        ...
        # 计算变异系数(CV) - 稳定性的关键指标
        quality['path_length_cv'] = (quality['path_length_std'] / quality['path_length_mean'] * 100).round(2)
        quality['planning_time_cv'] = (quality['planning_time_std'] / quality['planning_time_mean'] * 100).round(2)
        
        # 添加Sample-to-Success指标（如果有）
        if 'first_solution_iter' in df_success.columns:
            sts = df_success[df_success['pid_config'].isin(qualified['pid_config'])].groupby('pid_config').agg({
                'first_solution_iter': ['mean', 'std']
            }).round(2)
            sts.columns = ['samples_to_success_mean', 'samples_to_success_std']
            sts = sts.reset_index()
            quality = quality.merge(sts, on='pid_config', how='left')
        
        # 合并成功率和质量指标
        final = qualified.merge(quality, on='pid_config', how='left')
        
        print(f"\n✓ 第二层比较: 成功样本的综合性能指标")
        display_cols = ['pid_config', 'success_rate_pct', 'path_length_mean']
        if 'samples_to_success_mean' in final.columns:
            display_cols.append('samples_to_success_mean')
        display_cols.extend(['path_length_cv', 'planning_time_cv'])
        print(final[display_cols].to_string(index=False))
        
        # 第三层优化：综合排序（中等难度模式）
        # 排序策略：成功率(高) > 采样数(少) > 路径长度(短) > CV(小)
        sort_cols = ['success_rate']
        sort_asc = [False]
        if 'samples_to_success_mean' in final.columns:
            sort_cols.append('samples_to_success_mean')
            sort_asc.append(True)
        sort_cols.extend(['path_length_mean', 'path_length_cv'])
        sort_asc.extend([True, True])
        
        final_sorted = final.sort_values(sort_cols, ascending=sort_asc)
        ...
        print(f"  • 平均路径长度: {optimal['path_length_mean']:.2f} ± {optimal['path_length_std']:.2f} (CV={optimal['path_length_cv']:.1f}%)")
        print(f"  • 单位迭代路径效率: {optimal['path_efficiency_mean']:.4f} ± {optimal['path_efficiency_std']:.4f}")
        print(f"  • 平均规划时间: {optimal['planning_time_mean']:.2f}s ± {optimal['planning_time_std']:.2f}s (CV={optimal['planning_time_cv']:.1f}%)")
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"  平均路径长度: {optimal['path_length_mean']:.2f} ± {optimal['path_length_std']:.2f} (CV={optimal['path_length_cv']:.1f}%)\n")
            f.write(f"  单位迭代路径效率: {optimal['path_efficiency_mean']:.4f} ± {optimal['path_efficiency_std']:.4f}\n")
            f.write(f"  平均规划时间: {optimal['planning_time_mean']:.2f}s ± {optimal['planning_time_std']:.2f}s (CV={optimal['planning_time_cv']:.1f}%)\n")
