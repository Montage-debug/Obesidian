...
def get_model_path(model_filename="x-pose.pt"):
    ...
    # 兼容历史部署目录中的加密 PT 文件
    ws_root = os.getenv("WORKSPACE_DIR")
    if not ws_root:
        raise EnvironmentError("未设置环境变量 WORKSPACE_DIR")
    possible_enc_paths = [
...
def person_and_pose_judge(result, body_part_code):
    ...
    if body_part_code == "BP004":
        ...
        if (abs(body_angle) > 96.5 or abs(body_angle) < 83.5):
...
def draw_back_acupoints_on_original(img, acupoints_config, body_parrt_code, output_path="output_with_points.jpg"):
    ...
    shoulder_mid[0] = shoulder_mid[0] - 0.3 * PIXELS_PER_CUN_3

    """
    这里判断人的姿态是抬手的还是放平的, 如果是抬手的那吗将肩中点和胯终点更具肩宽大小进行下移
    肩宽小于185.0的时候，将肩中点下移0.5个单位，将胯终点下移0.5个单位，肩宽大于185.0的时候不做下移
    并重新设定PIXELS_PER_CUN的大小，让其变小,保持胯终点还是原位置。

    然后是手放平的情况就是，如果肩宽大于180.0的时候，将整体穴位上移动0.75寸
    如果170.0<肩宽<=180.0的时候，将整体穴位上移动0.25寸
    如果肩宽小于170.0的时候，不做处理
    """
    logger.info("肩宽像素级别:%2f", shoulder_width_conference)
    if left_wirst[1] < (hip_mid_y+shoulder_mid_y)/2 and right_wirst[1] < (hip_mid_y+shoulder_mid_y)/2:
        logger.info("人体手部状态：抬手")
        if shoulder_width_conference < 185.0:
            adjust_shoulder_mid = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN, offset_cun=-0.5)
            adjust_hip_mid = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN, offset_cun=-0.5, base_point="end")
            shoulder_mid[0:2]=adjust_shoulder_mid[0:2]
            hip_mid[0:2]=adjust_hip_mid[0:2]
            distance_pixels = distance_pixels - 0.5*PIXELS_PER_CUN
            PIXELS_PER_CUN = distance_pixels / 18.0
    else:
        logger.info("人体手部状态：放平")
        if shoulder_width_conference > 180.0:
            adjust_shoulder_mid = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN, offset_cun=0.75)
            adjust_hip_mid = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN, offset_cun=0.75, base_point="end")
            shoulder_mid[0:2]=adjust_shoulder_mid[0:2]
            hip_mid[0:2]=adjust_hip_mid[0:2]    
        elif shoulder_width_conference > 170.0 and shoulder_width_conference <= 180.0:
            adjust_shoulder_mid = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN, offset_cun=0.25)
            adjust_hip_mid = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN, offset_cun=0.25, base_point="end")
    ...
    right_start4 = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN_3, offset_cun=4.5, direction="right")
    right_end4 = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN_3, offset_cun=4.5, direction="right",base_point="end")

    right_start5 = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN_3, offset_cun=7.0, direction="right")
    right_end5 = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN_3, offset_cun=7.0, direction="right",base_point="end")

    right_start6 = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN_3, offset_cun=6.0, direction="right")
    right_end6 = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN_3, offset_cun=6.0, direction="right",base_point="end")
     #特殊穴位基准点
    ...
    Naoyu_start = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN_3, offset_cun=9.0, direction="right")
    Naoyu_end = calculate_acupoints_along_baseline(shoulder_mid, hip_mid, PIXELS_PER_CUN_3, offset_cun=9.0, direction="right", base_point="end")
...
def chose_body_part(img, body_part_code, output_path):
    ...
    if body_part_code == "BP004":
        ...
        back_acupoints_config = load_encrypted_json("acupoints_config.enc")
...
if __name__ == "__main__":
    input_image = f"./img_fix/head_hand1.jpg" #"./test_data/8.jpg"  # 请修改为实际路径
    ...
    success, status_code, message, acpoints_to_draw, shoulder_width, hip_width, img_with_points = chose_body_part(
        ...
        output_path=f"./img_fix_result/head_hand1.jpg"
    )
    ...
    # for i in range(1, 7):
    # # 指定你的背部图像路径
    #     input_image = f"./img_fix/no{i}.jpg" #"./test_data/8.jpg"  # 请修改为实际路径
    ...
    #         img=input_image,
    #         body_part_code="BP004",
    #         output_path=f"img_fix_result/no{i}.jpg"
    #     )
