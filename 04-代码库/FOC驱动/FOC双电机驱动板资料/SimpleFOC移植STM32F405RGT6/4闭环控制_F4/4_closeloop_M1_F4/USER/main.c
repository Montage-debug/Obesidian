...
int main(void)
{
	...
	voltage_sensor_align = 3;  	//V 航模电机设置的值小一点比如0.5-1，云台电机设置的大一点比如2-3
	...
	// 根据不同的模式判定，速度模式T100=100rad/s=954 RPM.力矩模式则T100为输出电压 = 100.
	//T6.28为旋转一圈
	
//| 控制模式          | T100含义         | 实际效果    |
//| ------------- | -------------- | ------- |
//| Type_torque   | 电机输出电压=100     | 类似给电机推力 |
//| Type_velocity | 目标速度=100 rad/s | 持续旋转    |
//| Type_angle    | 目标角度=100 rad   | 转到指定位置  |


	PID_velocity.P = 0.2;        //0.2, 速度环PI参数，只用P参数方便快速调试
	PID_velocity.I = 1;
