...
float current_limit;
unsigned long Index_needsSearch;
...
int alignSensor(void)
{
	...
	if(Index_needsSearch==1)exit_flag = absoluteZeroSearch();  //ABZ信号，查找机械零点
	if(exit_flag==0)return 0;    //机械零点查找失败，退出
...
int absoluteZeroSearch(void)
{
	...
	index_found=0;                  //在EXTI9_5_IRQHandler 中断函数中置1
	for(j=0;j<15;j++)               //最多转15对极，大概15——20秒。可以根据使用的电机极对数修改
	{
		...
		if(index_found==1)break;      //找到index，退出循环
	}
...
void setPhaseVoltage(float Uq, float Ud, float angle_el)
{
	...
	TIM_SetCompare1(TIM8,Ta*PWM_Period);
	TIM_SetCompare2(TIM8,Tb*PWM_Period);
	TIM_SetCompare3(TIM8,Tc*PWM_Period);
}
/******************************************************************************/
float velocityOpenloop(float target_velocity)
{
	...
	if(Ts <= 0 || Ts > 0.5f) Ts = 1e-3f; // quick fix for strange cases (micros overflow + timestamp not defined)
