...
void LowsideCurrentSense_Init(void)
{
	...
	ADC2_TRGO_Init();           //配置规则组中断，检测m1_phB，等待TIM8触发
	ADC3_TRGO_Init();           //配置规则组中断，检测m1_phC，等待TIM8触发
	
	TIM_Cmd(TIM8, ENABLE);      //使能TIM8，触发AD规则组转换
...
void Current_calibrateOffsets(void)
{
	...
	{
		if(_isset(pinA)) offset_ia += m1_phA;
		if(_isset(pinB)) offset_ib += m1_phB;
		if(_isset(pinC)) offset_ic += m1_phC;
		delay_ms(1);
	}
...
PhaseCurrent_s getPhaseCurrents(void)
{
	...
	current.a = (!_isset(pinA)) ? 0 : (m1_phA - offset_ia)*gain_a; // amps
	current.b = (!_isset(pinB)) ? 0 : (m1_phB - offset_ib)*gain_b; // amps
	current.c = (!_isset(pinC)) ? 0 : (m1_phC - offset_ic)*gain_c; // amps
