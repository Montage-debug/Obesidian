...
//	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_4;  //PC4/GPIO5，测试用于指示AD转换，测试完以后删除
//	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
//	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
//	GPIO_InitStructure.GPIO_Speed =GPIO_Speed_50MHz; 
//	GPIO_InitStructure.GPIO_OType=GPIO_OType_PP; 
//	GPIO_Init(GPIOC,&GPIO_InitStructure);
//	GPIO_ResetBits(GPIOC,GPIO_Pin_4);        //低
}
/*****************************************************************************/
//void adc1_deal(void)
//{
...
int main(void)
{
	...
	delay_ms(200);
	MagneticSensor_Init();
	LowsideCurrentSense(0.001f,20, NOT_SET,ADC_Channel_13,ADC_Channel_12);    //采样电阻阻值，运放倍数，A相，B相，C相
	LowsideCurrentSense_Init();  //配置ADC，使能TIM8，测量偏置电压
	LPF_init();                  //LPF参数初始化
	PID_init();                  //PID参数初始化
	...
	PID_current_d.P = 1;           //0.6 电流环PI参数，可以进入 PID_init() 函数中修改其它参数
	PID_current_d.I = 0;           //电流环I参数不太好调试，设置为0只用P参数也可以
	PID_current_q.P = 1;
	PID_current_q.I = 0;
	PID_velocity.P = 0.1;          //0.1, 速度环PI参数，只用P参数方便快速调试
	PID_velocity.I = 1;
	P_angle.P = 40;                //位置环参数，只需P参数，一般不需要改动
	PID_velocity.output_ramp = 100; //速度爬升斜率，如果不需要可以设置为0
	LPF_velocity.Tf = 0.01;
	target = 0;
	
	Index_needsSearch = M1_INDEX;    //如果使用ABZ编码器的Z信号=1
	if(Index_needsSearch==1) EXTI_Encoder_Init();   //如果使用Z信号，开中断
	Motor_init();
	Motor_initFOC(0,UNKNOWN);  //(3.25,CW); 第一次先获得偏移角和方向，填入代码编译后再下载，以后可以跳过零点校准。只用AB信号需要每次上电都零点校准。
	printf("Motor ready.\r\n");
