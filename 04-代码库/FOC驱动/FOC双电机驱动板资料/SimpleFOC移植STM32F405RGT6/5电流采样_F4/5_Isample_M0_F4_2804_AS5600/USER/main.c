...
int main(void)
{
	...
	voltage_power_supply = 12;   //V 电源电压，受电阻精度影响会有0.2V左右的误差
	printf("vbus=%.2f\r\n",voltage_power_supply);
	voltage_limit = voltage_power_supply/2;      //V，主要为限制电机最大电流
	pole_pairs=7;                //电机极对数，按照实际设置，虽然可以上电检测但有失败的概率
	voltage_sensor_align=3.5;    //V 航模电机设置的值小一点比如0.5-1，云台电机设置的大一点比如2-3
	velocity_limit=6.928; //此处填入值=当前电压/1.732          //rad/s 角度模式时限制最大转速，力矩模式和速度模式不起作用
...
//	PID_current_q.P=0.6;
//	PID_current_q.I=0;
	PID_velocity.P=0.12;         //速度模式0.1, 速度环PI参数，只用P参数方便快速调试
	PID_velocity.I=1;            //速度模式1
	...
	Index_needsSearch=M0_INDEX;         //如果使用ABZ编码器的Z信号=1
	...
	while(1)
	{
		...
		if(time_cntr>=1000000)  //us
		{
			...
			sprintf(snd2_buff, "Current=%.2f,Current=%.2f,Current=%.2f\r\n", currents.a, currents.b, currents.c);
			len = strlen(snd2_buff);
			USART2_SendDMA(len); 
		}
		move(target);
		loopFOC();
