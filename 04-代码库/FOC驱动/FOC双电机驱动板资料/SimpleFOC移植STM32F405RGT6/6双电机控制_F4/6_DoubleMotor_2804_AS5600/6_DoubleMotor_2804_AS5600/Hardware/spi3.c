...
void SPI3_Init_(uint8_t x, uint16_t spi_cpol)
{
	...
	//SPI中CSn引脚定义改这里以及MagneticSensorSPI.c
	if(x==0)
	{
		GPIO_InitStructure.GPIO_Pin=GPIO_Pin_8;  //PA5/GPIO1--CS0
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
		GPIO_InitStructure.GPIO_OType=GPIO_OType_PP;
		GPIO_InitStructure.GPIO_Speed =GPIO_Speed_100MHz;
		GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
		GPIO_Init(GPIOA,&GPIO_InitStructure);
		GPIO_SetBits(GPIOA, GPIO_Pin_8);     //CS0_H
	}
	else
	{
		GPIO_InitStructure.GPIO_Pin=GPIO_Pin_1;  //PA1/GPIO2--CS1
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
		GPIO_InitStructure.GPIO_OType=GPIO_OType_PP;
		GPIO_InitStructure.GPIO_Speed =GPIO_Speed_100MHz;
		GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
		GPIO_Init(GPIOA,&GPIO_InitStructure);
		GPIO_SetBits(GPIOA, GPIO_Pin_1);     //CS1_H
	}
