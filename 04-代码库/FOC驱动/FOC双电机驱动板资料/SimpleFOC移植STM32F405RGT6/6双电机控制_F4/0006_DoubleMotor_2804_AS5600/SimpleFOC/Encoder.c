...
uint16_t ReadABZ(uint8_t x)
{
	...
	{
		pulse_counter = TIM_GetCounter(TIM4);
...
void EXTI9_5_IRQHandler(void)
{
	...
	{
		TIM_SetCounter(TIM3,0);
		M0.index_found=1;                    //只在机械找零时有用，其它时刻没有意义
	}
	EXTI_ClearITPendingBit(EXTI_Line9); //清除LINE9 上的中断标志位
}
/******************************************************************************/
//M1_Z信号中断
void EXTI15_10_IRQHandler(void)
{
	...
	{
		TIM_SetCounter(TIM4,0);
		M1.index_found=1;                    //只在机械找零时有用，其它时刻没有意义
	}
	EXTI_ClearITPendingBit(EXTI_Line15);//清除LINE15 上的中断标志位
}
/******************************************************************************/
